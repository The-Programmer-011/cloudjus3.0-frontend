...
#Get-VMHost | where {($_.ConnectionState -eq "Connected") -and ($_.Name -ne "10.200.16.213")}
		...
		$lista = Get-Template -Location Templates -NoRecursion | where {($_.FolderId -eq "Folder-group-v77")} | Sort-Object name | ForEach-Object -Begin {$i=0} -Process {
			$i++
			"{0:D2}. {1}" -f $i,$_.Name
...
$stream = [System.IO.StreamWriter] "C:\DATABASE\vcloud\Lists\pastas.txt"
		$lista = Get-Folder -Type VM | where {($_.ParentId -eq "Folder-group-v3") -and ($_.Name.Contains("VMs"))} | Sort-Object name | ForEach-Object -Begin {$i=0} -Process {
			$i++
			"{0:D2}. {1}" -f $i,$_.Name
