...
if($_GET['nome_maquina']!=""){
    ...
    if(!$error){
      ...
      $snap = $_GET['snap'];
      header("Location: requests/Snapshots/_g_snapshot_manager.php/?nome_maquina=$vm&snap=$snap");
      echo "deu";
    }
  }
  if($name_not_found){
    $notification .= "new PNotify({ title: 'Instância não encontrada', text: 'A VM que você está procurando não existe.', type: 'error', styling: 'bootstrap3'});";
  }
  else if($unauthorized_machine){
    $notification .= "new PNotify({ title: 'Acesso não autorizado', text: 'Esta VM não pertence ao seu grupo.', type: 'error', styling: 'bootstrap3'});";
  }
?>
          ...
          <div class="">
            <div class="page-title">
              <div class="title_left">
                <h3>Criar Snapshot de uma VM</h3>
              </div>
            ...
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title">
                    <h2>Informe os dados para realização do snapshot e observe as instruções abaixo </h2>
...
<p> O snapshot é um arquivo de captura instantânea que registra um log de alterações dos dados de uma VM a partir do momento em que foi criado.</p>
<h5>a) O snapshot continua a crescer quando é mantido por um período mais longo. Isso pode causar falta de espaço no local de armazenamento e prejudicar o funcionamento do sistema de virtualização. <br><br> b) Assim, para um melhor desempenho, utilize no máximo 03 snapshots e mantenha-os por no máximo 72 horas. <br><br> c) Não use snapshots como backups. Após o período de 72h, uma rotina automatizada irá remover os snapshots ainda existentes. <br><br></h5> 

                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Nome da Instância (VM): <span class="required">*</span>
                        </label>
                      ...
                      <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="first-name">Nome do Snapshot: <span class="required">*</span>
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input name="snap" type="text" required="required" class="form-control col-md-7 col-xs-12" value="<?php echo $_GET['snap'];?>">
                        </div>
                      </div>
